...
{
	public function tableContact($param_list = array())
	{
		try
		{
			// 管理者状態でない場合はトップにリダイレクト
			if ($this->isAdmin() != "1")
			{
				header("Location: /");
				exit();
			...
			// 取得
			$sql  = "SELECT   `id`, `content`, `create_stamp` ";
			$sql .= "FROM     `contact` ";
			$sql .= "ORDER BY `id` DESC ";
			$contact_list = $this->query($sql);

			// 戻り値
			$return_list['contact_list'] = $contact_list;
	...
	public function tableStage($param_list = array())
	{
		try
		{
			// 管理者でない場合はトップにリダイレクト
			...
			$sql .= "        ,`stage`.`is_delete` ";
			$sql .= "FROM     `stage` ";
			$sql .= "ORDER BY `stage`.`id` DESC ";
			$stage_list = $this->query($sql);
	...
	public function tableCharacter($param_list = array())
	{
		try
		{
			// 管理者でない場合はトップにリダイレクト
			...
			$sql .= "          ,`character`.`image` ";
			$sql .= "FROM       `character` ";
			$sql .= "ORDER BY   `character`.`id` DESC ";
			$character_list = $this->query($sql);
